document.addEventListener("DOMContentLoaded", () => {
    ...
    // Sayfa Yüklendikten Sonra Filmleri Yükleme
        loadTrending();
        loadSoon();
    ...
    async function loadTrending() {
        try {
            const response = await fetch("movies/featured.json");
    ...
    async function loadSoon() {
        try {
            const response = await fetch("movies/soon.json");
            if (!response.ok) throw new Error("No Information");
            const data = await response.json();
    ...
    function initializeSoonSlider() {
        ...
        function updateSoonsPosition(withTransition = true) {
            if (withTransition) {
                soonsDiv.style.transition = "transform 0.3s ease";
            } else {
    ...
    async function loadMovies(genre) {
        try {
            const response = await fetch(`movies/${genre.toLowerCase()}.json`);
            if (!response.ok) throw new Error("No Information");
            const data = await response.json();
    
            const container = document.querySelector(".trendings");
            container.innerHTML = ""; // Clean previous movies
